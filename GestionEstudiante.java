...
public class GestionEstudiantes { 
    ...
    public void ordenarPorPromedioDescendente() {
         for (int i = 0; i < lista.size() - 1; i++) { 
            for (int j = 0; j < lista.size() - 1 - i; j++) { 
                if (lista.get(j).getPromedio() < lista.get(j + 1).getPromedio()) { 
    ...
    public Estudiante buscarPorCodigoBinario(int codigo) { 
        ...
        if (codigo < lista.get(medio).getCodigo()) { 
            fin = medio - 1; 
        } else 
        {     
            inicio = medio + 1; 
    ...
    public double sumaPromediosRecursiva(int indice) { 
        ...
        return lista.get(indice).getPromedio() + sumaPromediosRecursiva(indice + 1); 
        } 
    ...
    // MAYOR PROMEDIO RECURSIVO 
    // 
    public double mayorPromedioRecursivo(int indice) { 
        if (indice == lista.size() - 1) { 
            return lista.get(indice).getPromedio(); 
        } 
        double mayorResto = mayorPromedioRecursivo(indice + 1); 
        if (lista.get(indice).getPromedio() > mayorResto) { 
            return lista.get(indice).getPromedio(); 
        } 
        return mayorResto; 
        } 
